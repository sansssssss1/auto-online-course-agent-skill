...
- **验证式暂停恢复**（别乱点）：先确认当前是暂停态 → 单击一次播放键 → 3 秒后仍 `paused`
  ...
  单击播放键即恢复。
- **【2026-09-24 新增】小节卡片部件按需加载，视频任务点要先点开「N视频」页签**：
  进入小节时卡片**只渲染「1学习目标」部件**（此时 `mArg.attachments` 为空、卡片内 0 个 iframe、
  无 `<video>`，**都不代表本节没有视频**）；顶层部件栏 `li` 文本形如「2视频」，带序号好识别。
  不点它而走「附件数=0 → 全帧树轮询找 video」的兜底，会抓到**上一节的残留播放器**播满整段：
  信标 `t` 正常涨、日志正常打印「播放完成」，但**平台不计分**（该节任务点不动）。
  **播前身份校验**：`video.currentSrc` 含本节 `mArg.attachments[].objectId` 的片段才算播对片子
  （别用时长判断 —— 相邻节可能只差 1s）；起播用脚本 `v.play()`（`muted=false` + `volume=0.001`），
  不需要真实点击大播放按钮。播完核对章节树计数归零 + 平台计数 +1。
  完整配方见 knowledge/chaoxing.md §11.10。
...
### 4.6 测验（监督者实操，2026-09-18 首战验证；详细选择器与坑见 knowledge/icourse163.md §8）

Runner 不代答。监督者用 playwright-core 无头 + 同 `MOOC_PROFILE_DIR` **常驻单会话**作答
（脚本轮询指令文件，答案由监督者读题后写入；**同一 profile 严禁双开浏览器**，双开 =
僵尸会话 + 服务端并发节流）。流程与词表：

1. **查清单**：`#/learn/testlist`（测验只在此处，课件列表无测验任务点）+「考试」tab；
   记录每门测验的「可尝试次数 N 次（取最高分）/测验限时/截止时间/有效提交次数 n/N」。
2. **读规则横幅**：quiz 页顶部（`#/learn/quiz?id=…`），点开始前即可读全。
3. **开始闸门**：勾诚信复选框 `input.j-agree`（配方 `el.checked=false; el.click()`，只设
   checked 不够）→ `.j-startBtn` 的 `u-btn-disabled` 消失 → 点「开始测验」**即计时**。
4. **读题作答**：题干 DOM 明文无混淆；每题 `.m-choiceQuestion`，选项为原生
   `input.u-tbi` radio/checkbox（DOM 顺序=A/B/C/D，点 radio 本身并校验 `checked`）。
   **程序化点选每题间隔 ≥1–2 秒**——实测 20 题 2 秒连点触发「对不起，请稍后再试」节流，
   交卷确认层反复弹出且模态框叠层 216 个节点。
5. **交卷**：`.j-submitBtn`（「提交答案」）→ 确认弹层点**可见且精确文本「确定」**的按钮
   （类名不固定，勿按类名找）。被节流时：等 4–5 分钟 → remove 全部残留 `.ux-modal` →
   重走提交流。成功判据 = hash 切 `#/learn/quizscore?…` + 「本次得分为：…」。
6. **结果页与重做口径（与学习通相反，按课程探测）**：正解与得分**可能被教师隐藏**
   （`analysisMode` 只有我的答案，分数 span 为空）→ 无修正依据时**首答尽力、不盲目重做**；
   正解可见时按安全规则第 3 条「再做一次」修正到高分。剩余次数一律保留给用户决策。
...
**中国大学MOOC**（**2026-09-18 已实测**，选择器与坑见 knowledge/icourse163.md §8、
流程词表见 SKILL.md §4.6）：测验在 `#/learn/testlist`，规则横幅写明限次/限时；
**结果页正解与得分可能被教师隐藏（实测隐藏）→ 无修正依据时首答尽力、不盲目重做**；
正解可见则按重做口径修正。Runner 不自动做测验。
...
- ✅ **中国大学MOOC·Runner**：视频/文档流程实战验证（自动发现、扫码登录、看门狗）；
  **2026-09-14 马原课新周 7 视频 + 7 文档全部 learned（14/14）**；**2026-09-18 测验流程实测打通**
  （马原第一章单元测验 20 单选一次交卷，有效提交 1/3；教师未公布正解 → 不盲目重做；
  并发节流与模态叠层两坑已入册，见 knowledge/icourse163.md §8 与 §4.6 词表）
