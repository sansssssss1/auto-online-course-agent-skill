...
- **【2026-09-24 修正】弹题寿命不是固定 1.5~3 分钟**：`cx_quiz_log` 实测两例为 `heldSec=362`
  （未答而随卡片销毁）与 `546`（挂满 9 分钟后被人工答掉）——窗口随课程/题量变化，
  别把 3 分钟当硬上限（9~10 分钟一轮的巡检仍可能抓到）。但「未答上可能作废该节视频任务点」
  的风险不变 → 仍按「出现即答」执行；**题库命中的题由 bridge 在 2s 内自动作答**才是保险。
...
### 11.10 卡片部件按需加载：「1学习目标 / 2视频 / 3章节测验」页签要先点开【实测 2026-09-24，重要】

- 结构：学习页（顶层文档）部件栏是一行 `li`，**文本带序号**（原文 `1学习目标 2视频 3章节测验`），
  形如 `onclick="changeDisplayContent(2,3,'{kid}','{courseid}','{clazzid}','')"`。
- **关键事实**：进入小节时卡片**只渲染第 1 个部件（学习目标）**，此时
  `mArg.attachments === []`、卡片内 0 个 iframe、无 `<video>` ——
  **「attachments 为空」不代表本节没有视频**。点「2视频」后 `attachments` 立刻变为
  `[{type:'video', objectId:…}]`，并出现 `iframe.ans-attach-online.ans-insertvideo-online`
  （`ananas/modules/video/index.html`）。
- **后果（实测踩坑，该节白播一整段）**：靠「附件数=0 → 全帧树轮询找 video」的兜底会命中
  **上一节遗留的旧播放器**并在错误的 video 上播满整段：信标 `t` 正常增长、「播放完成」正常打印，
  但**平台侧没有本节心跳、任务点不动**（该节不计分，章节树计数不降）。
- **播前必做身份校验**：`video.currentSrc` 必须含本节 `mArg.attachments[].objectId` 的片段
  （本例 `…/b28ddb210ba0dee246d37/sd.mp4?at_=…`）；**不要用「时长相同/接近」当判据** ——
  同一课程相邻两节时长可能只差 1s（1324s / 1323s）。
- 起播口径：首帧 `readyState=0`、`networkState=1`、大播放按钮 `.vjs-big-play-button` 可见；
  **脚本 `v.play()`（`muted=false` + `volume=0.001`，同 §11.2 省电暂停口径）即可加载并起播**，
  不需要真实鼠标点击（Playwright 对 `.vjs-big-play-button` 的 actionability 点击会超时）。
- 完成判据：播完该节章节树「N个待完成任务点」立即归零（1→0）+ 平台「已完成任务点」+1。
