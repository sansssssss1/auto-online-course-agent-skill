...
> 2026-09-17 下午实战验证（课程② 28→35/68）+ 修正：
> - **beacon 信标 ✅**：全流程有效——整门课一次注入；ENDED/QUIZ/停摆自动识别；
>   4.1 以 `t=1668/1668,p=1,e=1` 精准收尾；停摆诊断用 `i` 计数增长定位。交接模板 ✅ 无缝。
> - **自适应巡检 v1 假设被推翻**：调用总数 = 工具上限(~120s) × 播放时长，调用内放大间隔无效
>   → **阶梯 v2**：干净轮次后拉长**轮间距**至 5–10 分钟（目标 400+ → ~100–150 次调用）。
> - **字体映射自举判负**：混淆按页随机（同字跨页映射不同：题→嶋/戮），页内自举 30 条
>   跨页复用命中 0/218 → 全局映射不成立，降级页内辅助。
> - **新发现 IAB 前台红线**：同会话切标签 → 视频标签停摆（visibility 仍 visible，
>   readyState/error 正常）；恢复 = 关其他标签再激活；健康判据 = `t` 推进而非 `hasFocus()`。
> - **测验策略升级（用户授权"尽量拿高分，自行推进"）**：0 分提交不点亮任务点（课程② 3.7
>   样本）→ 用重做入口按错题台账自行修正，无需逐次询问。
> - 凭据自助登录未触发（本轮未掉线）；登录探针误报已修正（页面骨架判断）。

> 2026-09-17 晚实战（课程②收尾 **68/68**，本轮 36→68 全部 100 分）：
> - **重做策略大获成功**：提交后平台明文列出每题正确答案 → 首答不纠结、读明文重做一次即满分
>   （4.3/5.1/5.2/5.3/6.1/6.5/8.1/8.2/8.3 全部修正到 100）。致命细节 = 重做**保留上次作答**，
>   修正必须"精确设定集合"（每选项既加选也取消）；qid 顺序须在**首次提交前**采集
>   （提交后 `input[name^=answer]` 被移除）。
> - **阶梯 v2 实测**：单次 JS 调用上限 ~115–118s（`waitForTimeout(300000)` + 120s 工具上限
>   实测 abort）；调用数下界 = 播放秒数 ÷ ~115（1748s 视频 = 15 次调用）；STEP 放大
>   5500→15000ms、收到 `e=1` 就地推进（结束检测 + quiz 打开同调用完成）→ 调用数明显下降。
>   **"5–10 分钟一查"单会话做不到 → 候选方案：轮间用 Bash `sleep 300` 过渡（近乎零 token），
>   下轮验证。**
> - beacon 一次注入**整门课存活**（localStorage 持久化、页内切小节存活）。
> - 新坑：静默暂停恢复须**跨帧**找 `.vjs-play-control`（顶层 querySelector 恒 null，
>   曾因 rec++ 假象白等 ~110s）。
> - 课程③交接模板 `docs/templates/continue-prompt-20260918.md` 已产出；本轮代码零改动。
>   token 主要耗在等待调用；OCR 读题中文偶有乱码但不影响答题判断。

> 2026-09-18 收官（课程③ **67/67** 完成，三门课 195/195 全部跑完）：
> - **9.10 期末测验首提 100 分**（含 1 道图形选项题——新配方：页内 fetch 选项 img →
>   合成 2×2 网格 canvas → 单图 vision 读）；全课 8/9 测验首提满分，仅 6.5=75
>   （无重做入口、及格线 0，按安全规则接受推进，是否人工补救由用户决定）。
> - **sleep 模式全程定稿**：全课 ~450–480 次调用 vs 纯轮询估算 670+（**省 ~30%**；
>   干净段省 2/3、弹题常态省 1/4–1/3）；"5–10 分钟一查"在单会话内仍不可行（120s 工具上限），
>   但 Bash sleep 使模型侧 token 近零。视频弹题 ~30 道，首答 28/30，2 道经 qw/重答闭环。
> - **凭据自助登录获实战验证**：一次「提交卡死 + 掉登录」事故中，整页刷新至 passport2 后
>   用交付凭据自助重登，视频从服务器心跳位置续播——零人工介入。
> - **新配方 3 条**（field-notes 2026-09-18 晚）：① 合法 seek 补结尾弹题（仅限已完成节点）；
>   ② 无重做课程口径：任务点点亮即接受当前分数；③ 图形选项题 vision 读法。
>   err=3 解码错误与 err=2 同配方可恢复。
> - 全程零验证码、零日上限、零伪造请求。阶段三（本 PR）为收官动作。
